...
require_once($_SERVER["DOCUMENT_ROOT"] . '/cabinet/core/unlogined_booking_details_page.php');
...
if (isset($_GET['BookingID'])) {
    ...
    if (preg_match('/^[1-9][0-9]*$/', $BookingID)) {
        ...
        $Querry = $mysqli->prepare("
              SELECT c.BookingID, c.UnloginedShow
                FROM Booking c
                WHERE c.BookingID = ?
                LIMIT 1;
            ");
        ...
        //
        if (count($result) > 0) {
            if ($result[0]['UnloginedShow'] == 1) {
                $page = new unlogined_booking_details_page($mysqli, $BookingID);
                $page->generate();
                echo $page->CONTENT;
            } else {
                header('location: /');
