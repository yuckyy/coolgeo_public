...
Class contacts_page
{
    ...
    //конструктор
    function contacts_page($mysqli, $PageID, $Lang)
    {
        $this -> mysqli = $mysqli;
        $this -> PageID = $PageID;
        $this -> Lang = $Lang;
    ...
    function generate($TopMenuActive = 0)
    {
        ...
        $this -> page_templete = $translator->translate($this -> page_templete);
    ...
    private function GetMainPageTemplete(){
        $_file_path = $_SERVER["DOCUMENT_ROOT"].'/cabinet/templete/pages/contacts_page.html';
        if (file_exists($_file_path))
        {
